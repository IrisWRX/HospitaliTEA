    ...
    map.on('load', () => {
        ...
        map.loadImage(
          ...
          (error, image) => {
            ...
            db.collection('shelters').get().then(allEvents => {
              ...
              features.push({
                'type': 'Feature',
                'properties': {
                  'description': `<strong>${event_name}</strong><p>${preview}</p> <br> <a href="/information?id=${doc.id}" target="_blank" title="Opens in a new window">Read more</a>`
                },
                'geometry': {
                  'type': 'Point',
